...
/* 	 If No Order Taker with waiting status(2) is found then its the 
	...
	/* @ */
	Print_1Arg("Customer %d wake up after place order and ready to give money\n$", myIndex);
...
/*	ADD	Daniel */		 
		...
		/* 		@
				Print_Stmt("incrementing the foodToBeCookedData.sixBurger\n$"); */
				foodToBeCookedData.sixBurger++;
			}
			if(custData[custIndex].threeBurger == 2){
		/* 		@
				Print_Stmt("incrementing the foodToBeCookedData.threeBurger\n$"); */
				foodToBeCookedData.threeBurger++;
			}
			if(custData[custIndex].vegBurger == 2){
	/* 			@
				Print_Stmt("incrementing the foodToBeCookedData.vegBurger\n$"); */
				foodToBeCookedData.vegBurger++;
			}
			if(custData[custIndex].fries == 2){
		/* 		@
				Print_Stmt("incrementing the foodToBeCookedData.fries\n$"); */
				foodToBeCookedData.fries++;
			}
/*	ADD	Daniel */ 
...
{	
	...
	/* @ */
/* 	Print_1Arg("Cook got his index %d\n$", myIndex); */
	...
	while(1){	
		...
		/*@*/
/* 		Print_1Arg("cook %d starts working\n$",myIndex); */
...
/* 		Print_1Arg("cook know his type is %d\n$",whatToCook); */
	...
	/* 			Print_1Arg("cook know his type is %d\n$",whatToCook); */
		...
		if(whatToCook == VEGBURGER){
			/* OG*/
			Print_1Arg("Cook %s is going to cook veggie burger\n$", myIndex);	 
			foodReadyData.vegBurger++; 
		}
		
		if(whatToCook == FRIES){
			/* OG*/
			Print_1Arg("Cook %s is going to cook french fries\n$", myIndex);	
			foodReadyData.fries++;  
		}	
		...
		/* Doubt */
	/* 	Print_1Arg("Cook %d is going to Yield now\n$", myIndex); */
		Yield(); 
	/* 	Print_1Arg("Cook %d is back from Yield now\n$", myIndex); */
	}
} 
...
void Manager()
{
    ...
    while(1){
		...
		/* @ */
		Print_Stmt("Manager Thread entered2\n$");
		Release(custServedLock);
		tablesFree = 0;
		...
		if(inventory <= MININVENTORY){
			...
			Print_Stmt("Manager find inventory is empty\n$");
			Acquire(moneyAtRestaurantLock);
		...
		for(index = 1; index < 5; index ++){
			Print_Stmt("Manager ckecking for all items\n$");
