...
// Set up all registers and switch to user mode to run the userprog
	...
	machine->WriteRegister(StackReg, processTable[myPid].stackTops[currentThread->threadId]);
	// @
//	printf("Kernel func : thread id is %d name is %s, stacktop is %d\n", currentThread->threadId, currentThread->getName(), processTable[myPid].stackTops[currentThread->threadId]);
...
void Fork_Syscall(unsigned int vaddr) {
	...
	/* //--------
 	// remove after exec is implemented
	processTable[0].addrSpace = currentThread->space;
	processTable[0].Pid = 0;
	if(onlyOnce == 0){
	processTable[0].threadCount = 0;
	onlyOnce = 1;
	} 
	processTable[0].stackTops[0] = ((currentThread->space->GetNumPages()) * PageSize) - 16;
...
//		printf("in Fork found phyPage is %d\n", freePageIndex);
	...
	//void SetAddrSpace(AddrSpace *newSpace);	
...
//	printf("threadCount in fork is %d\n", processTable[myPid].threadCount);
	...
	// @
//	printf("Thread id assigned is %d\n", t->threadId);
	processTable[myPid].stackTops[processTable[myPid].threadCount] = ((newNumPages * PageSize) - 16);
	// @
//	printf("stack top is %d\n", ((newNumPages * PageSize) - 16));
	processTableLock->Release();
...
void Exit_Syscall(int status) {
		...
		/* printf("thread %d entering exit , name is %s\n", currentThread->threadId, currentThread->getName());
		currentThread->Finish(); */
		
	 	// Exit System call Local variables
		
		int indexi, indexj, index = 0, countPhysPages = 0, localNumPages = 0, myProcess = 0;
		int setIfLastThread = 0, stackTopValue = 0;
		unsigned int pageNo = 0, currentThreadId = 0, vaddr[8], physicalPages[8];
		TranslationEntry* myPageTable;
		// @
		printf("thread %d entering exit , name is %s with status %d\n", currentThread->threadId, currentThread->getName(), status);
		// End of Exit System call Local variables	 
		
		if(currentThread->threadId == -1){
			// @
		//printf("I am main so I am finishing from exit syscall\n");
			// @
			//printf("The Main function is calling Finish\n");
			currentThread->Finish();
			return;
		}
			
		// finding the thread which invoked exit and verifying the address space of thread and process
		// and storing the process number in myProcess variable
			processTableLock->Acquire();
			for( indexi = 0 ; indexi < processCount; indexi++)
			{
				if(processTable[indexi].addrSpace == currentThread->space) 
					{	
						myProcess = indexi;
						// @
						printf("My process id is %d thread count is %d\n", myProcess, processTable[myProcess].threadCount);
						processTable[myProcess].threadCount--;
						if(processTable[indexi].threadCount == 0)
						{
							setIfLastThread = 1;
							// @
							printf("Yes I am the last thread in this process %d\n", myProcess);
							break;
						}
						// @
						printf("ThreadCount in this process is %d\n", processTable[indexi].threadCount);
					}
			}
			processTableLock->Release();
			// checking to see if this is the last thread in nachos
			// @
//			printf("Current number of processes = %d\n", noOfActiveProcesses);
			if(noOfActiveProcesses == 1) // exec must increment this value
			{
				processTableLock->Acquire();
				...
				for( indexi = 0 ; indexi < processCount; indexi++)
				{
					...
					{	
							if(processTable[indexi].threadCount == 0)
							{
								...
								// @
								printf("I am halting the machine\n");
								interrupt->Halt();
							}
					}
				}
			...
			if(setIfLastThread == 1)
			{
				//	deallocate all valid entries in page table , delete address space if last thread in process
				// but not in NACHOS
				
				// get the reference to the page table 	
				myPageTable = currentThread->space->GetPageTableRef();
				// @
				printf("last thread in this process so deallocate pagetable and clear phymem\n");
				localNumPages = currentThread->space->GetNumPages();
				// @
//				printf("Number of pages in the process addrSpace = %d\n", localNumPages);
				for( indexi =0 ; indexi < localNumPages; indexi++) // numpages is private need to access it using a function
				{						
					pageTableLock[myProcess]->Acquire();
					pageNo = myPageTable[indexi].physicalPage;
					pageTableLock[myProcess]->Release();
					// @
//					printf("Clearing the phymem index = %d\n", pageNo);
					// Acquire lock on bitmap object
					bitMapObjectLock->Acquire();
					// Clear the value in the bitmap
					bitMapObject->Clear(pageNo);
					// Release lock on bitmap object
					bitMapObjectLock->Release();
				}
				// delete the address space, destructor in addressspace deletes pagetable
					
				activeProcessesLock->Acquire();
				noOfActiveProcesses--;
				// @
//				printf("decrementing the process count so new val = %d\n", noOfActiveProcesses);
				activeProcessesLock->Release();
		...
		{
			//  if a thread calls exit and it's not the last thread in the process deallocate it's stack
			// @
			printf("not last thread so deallocate only stack\n");
			// fetching the current Id
			currentThreadId = currentThread->threadId;
			// @
//			printf("Thread id in exit func is %d\n", currentThreadId);
			// fetching the the vaddr of the current thread stack top
			stackTopValue = processTable[myProcess].stackTops[currentThreadId];
			stackTopValue += 16;
			// @
//			printf("stack top is %d\n", stackTopValue);
			for(indexj = 1; indexj <= 8; indexj++)
			{
				vaddr[indexj] = ((stackTopValue - PageSize)/PageSize);
				// @
//				printf("vaddr to be deleted = %d\n", vaddr[indexj]);
				stackTopValue= stackTopValue - PageSize;
			}
			myPageTable = currentThread->space->GetPageTableRef();
			pageTableLock[myProcess]->Acquire();
	
			// looping pagetable to find the physical address
			for(indexj = 1; indexj<= 8; indexj++)
			{
				physicalPages[indexj] = myPageTable[vaddr[indexj]].physicalPage;
				// @
//				printf("vaddr to be deleted = %d\n", vaddr[indexj]);	
				// @
//				printf("PhyAddr to be deleted = %d\n", physicalPages[indexj]);
			}
			...
			for(indexi = 1; indexi <= 8; indexi++)
			{
				// Clear the value in the bitmap
				// @
//				printf("Last else, clearing the phyMem page no = %d\n", physicalPages[indexi]);
				bitMapObject->Clear(physicalPages[indexi]);
			
			}
			// Release lock on bitmap object
			bitMapObjectLock->Release();
			// @
			//printf("I am before finish in exit\n");
			currentThread->Finish();
		} 
}			
