...
int flag = 0;
int foodToBagAlreadyIncremented[100];
...
/* 	 If No Order Taker with waiting status(2) is found then its the 
	...
	if(custData[myIndex].dineType == 1){
		...
		if(custData[myIndex].delivered == 1){
			...
			Print_1Arg("To Go Customer %d calling E---X---I---T afterorder\n$", myIndex);
			Exit(0);
		...
		if(tableAvailable == 1){		
			...
			if(custData[myIndex].tableNo != 0){	
				...
				while(1){
					...
					Print_2Arg("customer %d is  at table number %d Waiting for food\n$",myIndex, custData[myIndex].tableNo);
					Wait(eatInFoodWaitingCV,eatInFoodWaitingLock);
					Print_1Arg("EATINcustomer %d is  wakedup by waiter\n$",myIndex);
...
/* ADD Daniel */
	...
	/* @ */
	Print_1Arg("CUSTOM %d goes to WRONG WAY----------------\n$",myIndex);	
}
...
int BagTheOrders()
{
	...
	Print_Stmt("Ordertaker inside bag the orders func\n$");
	...
	for(index = 1; index <= CUST_COUNT; index++){
		...
		{
			/* @ */
			Print_1Arg("Inside bag the orders, inside bagged != 1 for cust = %d\n$", index);
			if(custData[index].sixBurger == 1)
			{
				Print_1Arg("Customer %d ordered 6 burg\n$", index);
				Release(customerDataLock);
				...
				/* @ */
				Print_2Arg("Inside bag order: cannot be bagged = %d, for cust %d\n$", cannotBeBagged, index);
				if(cannotBeBagged == 1)
				{
					continue;
					/* @ */
					Print_Stmt("I should not Print\n$");
				...
				{					
					...
					foodToBag-- ;
					/* @ */
					Print_2Arg("Bag the orders decrementing the food to Bag count to %d for customer %d\n$", foodToBag, index);
		...
		{
				/* @ */
				Print_1Arg("cust = %d order already bagged\n$", index);
				Release(customerDataLock);
		}
	}	
	return -1;
...
void OrderTaker()
{
	...
	while(1)
	{
		...
		/* @ */
		Print_1Arg("OrderTaker %d begins to do his job\n$", myIndex);
		if(custLineLength > 0){
			/* customers are available so I am releasing the food to bag lock	*/
			/* @ */
			Print_1Arg("OrderTaker %d had a customer in line to serve\n$", myIndex);	
			...
			/* @ */
			Print_1Arg("OrderTaker %d starts bagging the food\n$", myIndex);
			Release(custLineLock);
			/* Acquire the foodToBagLock to check if there are any orders to bag	*/
			Acquire(foodToBagLock);
		
			if(foodToBag > 0){
				Print_2Arg("OrderTaker %d find foodToBag is %d\n$", myIndex, foodToBag);
				...
				index = BagTheOrders();
				Print_2Arg("Ordertaker %d after bagged the order func call and index is = %d\n$", myIndex, index);
				if(index > 0){
					/* OG	*/
