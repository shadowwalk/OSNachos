...
PostOffice::Send(PacketHeader pktHdr, MailHeader mailHdr, char* data)
{
	...
	{	char *buf = new char[40];
		...
		int packetNo=atoi(temp);
	
		if((pktHdr.from != 29))          
		{
			//Adddint to queue
			pktHdr.from=clientId;
			
			if(!((strcmp(content,"ACK") == 0)) )
			{	
				//sprintf(sendMsg,"%d%c%d", content, separator, pktHdr.to);
				
				msgQueue->Append((void *)buf1);
	...
	// -----------------------------------------------------------------
	//	CODE ADDED TO MAKE THE POSTOFFICE SEND A BIT SLOW
	// -----------------------------------------------------------------
